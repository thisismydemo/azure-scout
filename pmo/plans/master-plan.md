...
5. ✅ **Wire the module manifest** (AB#5024) — the assessment core/`Test-ScoutPermission` are loaded; `src/` dot-sourced. _Committed `ddebe27`._
...
| 2026-07-21 | Added per-domain CAF/WAF analytics prototype (Epic AB#5056): domain ARG collection, 15 category assessments + sub-bundles in the manifest, 10 rule files, registry doc; wired `src/` into the module as the assessment implementation. |
...
| 2026-07-22 | Recorded the §9a reporting-solution decision: [`decisions/pptx-renderer.md`](../../docs/design/decisions/pptx-renderer.md) recommends OpenXML SDK over python-pptx/COM, grounded in the current `Export-Pptx.ps1`/`build_deck.py` prototype and the `ubuntu-latest` CI agent. Flagged as a drafted recommendation pending repo owner sign-off — not a unilateral final call. §9a reframed from "open, blocks AB#5044" to "recorded, no longer blocking". |
