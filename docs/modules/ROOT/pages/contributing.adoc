...
. Use `Invoke-AZSCGraphRequest` for the Graph API call:
+
[source,powershell]
----
function Get-AZSCNewEntraObject {
    param(
        [Parameter(Mandatory)]
        [string]$TenantID
    )

    $results = Invoke-AZSCGraphRequest -Uri '/newEndpoint' -Method GET
...
. Register the Graph query in `Start-AZSCEntraExtraction`.
...
BeforeAll {
    ...
    Import-Module "$ModuleRoot/AzureScout.psd1" -Force
}

Describe 'Get-AZSCNewResource' {
    It 'returns normalized objects' {
        Mock Invoke-AZSCGraphRequest { return @(@{ id = '1'; displayName = 'Test' }) }
        $result = Get-AZSCNewEntraObject -TenantID '00000000-...'
...
* Functions use the `Get-AZSC` / `Invoke-AZSC` / `Set-AZSC` / `Start-AZSC` / `Test-AZSC` prefix
...
* Module version (`(Get-Module AzureScout).Version`)
