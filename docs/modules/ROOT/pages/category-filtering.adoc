...
. Update xref:category-structure.adoc[Category Structure] with the new mapping
. Create one or more `.ps1` module files using the module template in `Modules/Public/InventoryModules/`

== See Also

* xref:category-structure.adoc[Category Structure reference]
* xref:coverage-table.adoc[Full Coverage Table]
* xref:ROOT:index.adoc[AzureScout Documentation Home]
