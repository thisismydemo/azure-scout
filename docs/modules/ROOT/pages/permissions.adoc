...
:description: ARM RBAC and Microsoft Graph permissions required by Azure Tenant Inventory.
:keywords: permissions, rbac, graph, api, roles

== Overview

Azure Tenant Inventory requires two categories of permissions:
...
The `Test-AZTIPermissions` function runs automatically before extraction (unless `-SkipPermissionCheck` is set):
...
Test-AZTIPermissions -TenantID '00000000-...' -Scope All
----
