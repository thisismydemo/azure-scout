<#
.Synopsis
Inventory for Azure AI Foundry Projects

.DESCRIPTION
This script consolidates information for all Azure AI Foundry Project resources
(microsoft.machinelearningservices/workspaces where kind == 'Project').
Excel Sheet Name: AI Foundry Projects

.Link
https://github.com/thisismydemo/azure-inventory/Modules/Public/InventoryModules/AI/AIFoundryProjects.ps1
...
{
    $projects = $Resources | Where-Object {
        $_.TYPE -eq 'microsoft.machinelearningservices/workspaces' -and $_.KIND -eq 'Project'
    }

    if ($projects) {
        $tmp = foreach ($1 in $projects) {
            $ResUCount = 1
...
{
    if ($SmaResources) {
        $TableName = ('AIFoundryProjectsTable_' + (($SmaResources.'Resource U' | Measure-Object -Sum).Sum))
